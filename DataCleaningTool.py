...
# SECTION 2: Data Overview
    ...
    df = st.session_state.processed_df  # Add this line
    ...
    if st.button("Run Outlier Detection"):
        ...
        if outlier_report:
            ...
            for col, stats in outlier_report.items():
                ...
                with c2:
                    if st.button(f"🗑️ Drop rows with outliers in '{col}'", key=f"drop_{col}"):
                        df = df[~stats["mask"]]
                        st.session_state.processed_df = df
...
# SECTION 9: Impute Missing Values
    ...
    if df is not None:
        ...
        if total_missing == 0:
            ...
            if st.checkbox("🗑️ Drop rows with more than 50% missing values"):
